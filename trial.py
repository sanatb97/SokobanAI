...
class moveableObs(object):
	...
	def handleKeyPress(self,gameMap,sleeptime=0):
		'''
		sets target based on keypress, and calls moveTo which takes further decision/action.
		'''
		currCol=int(self.x/50)
		currRow=int(self.y/50)
		horizontal=1
		...
		if event.key==pygame.K_LEFT:
			...
			targetRow=currRow
			offset=-50
			return self.moveTo(targetRow,targetCol,currRow,currCol,offset,horizontal,gameMap,sleeptime)
	...
	def moveTo(self,targetRow,targetCol,currRow,currCol,offset,horizontal,gameMap,sleeptime=0):
		'''
		...
		if(target==-1):
			...
			if(target<=-10):
				boulderIndex=int(target/10*-1-1)
			else:
				boulderIndex=int(target)

			moveTheMan=boulderlist[boulderIndex].handleKeyPress(gameMap)
			if(moveTheMan==1):
				#change gameMap
...
def revertToStart(gameDisplay,level,boulderlist,width,height):
	'''
	resets number of boulder and boulderOnGoal.
	empties boulderlist.
	builds gameMap from beginning and initialises GUI board.
	returns the new gameMap and manOb.
	'''
...
def doAImoves(moves):
	for move in moves:
		...
		manOb.handleKeyPress(gameMap,1)
		pygame.display.update()
			...
			for event in pygame.event.get():
				...
				if event.type==pygame.KEYDOWN:
					if event.key==pygame.K_r:
						gameMap,manOb=revertToStart(gameDisplay,level,boulderlist,width,height)
						#print(gameMap)
						break
					
					elif event.key==pygame.K_a:
						t0=time.time()
						cmd="python2 sokoban.py --level="+level+" --last_level="+str(int(level)+1)
						print(os.system(cmd))
						solfile=open('sol.txt','r')
						doAImoves(solfile.read())
						print("\n"+"Time taken : "+str(time.time()-t0)+" seconds")
						pass
					else:
						manOb.handleKeyPress(gameMap)
						#print(gameMap)
